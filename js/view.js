...
view.showNotification = function(tagID) {
    ...
    for (let noti of model.notifications) {
        document.getElementById(tagID).innerHTML += `<div style="display: flex; justify-content: space-between; border: 1px solid #E0E3EB;border-radius: 10px;margin: 3px;" ><div style="display: flex;align-items: center;padding: 5px;"><p>${noti.createdAt} : ${noti.content}</p></div> 
        <div style="display: flex;align-items: center;padding: 5px;"><button name="${noti.id}" class=" Button-Notification delete-noti" > Xóa </button></div></div>`
